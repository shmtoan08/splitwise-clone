...
const resetEmailTranslations: Record<string, EmailContent> = {
  vi: {
    subject: "Đặt lại mật khẩu của bạn - Wari App",
    senderName: "Wari App",
    title: "Đặt lại mật khẩu",
    greeting: "Xin chào,",
    body: "Chúng tôi nhận được yêu cầu đặt lại mật khẩu cho tài khoản Wari App của bạn. Vui lòng click vào nút bên dưới để tạo mật khẩu mới:",
    buttonText: "Đặt lại mật khẩu",
    fallbackText: "Hoặc copy đường link này vào trình duyệt:",
    expiryNotice: "Link đặt lại mật khẩu này sẽ hết hạn sau 1 giờ. Nếu bạn không yêu cầu đặt lại mật khẩu, vui lòng bỏ qua email này.",
  },
  ja: {
    subject: "パスワード再設定のご案内 - Wari App",
    senderName: "Wari App",
    title: "パスワードの再設定",
    greeting: "こんにちは、",
    body: "Wari Appアカウントのパスワード再設定リクエストを受け付けました。以下のボタンをクリックして新しいパスワードを設定してください：",
    buttonText: "パスワードを再設定する",
    fallbackText: "または以下のリンクをブラウザに貼り付けてアクセスしてください：",
    expiryNotice: "この再設定リンクの有効期限は1時間です。心当たりがない場合は、このメールを破棄してください。",
  },
};

export async function sendPasswordResetEmail(email: string, token: string, locale: string = "vi") {
  const currentLocale = locale === "ja" ? "ja" : "vi";
  const content = resetEmailTranslations[currentLocale];

  const domain = process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000";
  const resetLink = `${domain}/${currentLocale}/new-password?token=${token}`;
  const fromSender = process.env.EMAIL_FROM || `${content.senderName} <onboarding@resend.dev>`;
  ...
  return await resend.emails.send({
    ...
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; line-height: 1.6; color: #1e293b; max-width: 560px; margin: 0 auto; padding: 24px; border: 1px solid #e2e8f0; border-radius: 16px; background-color: #ffffff;">
        ...
        <p style="font-size: 15px; margin-bottom: 24px; color: #475569;">${content.body}</p>
        <div style="text-align: center; margin: 28px 0;">
          <a href="${resetLink}" style="background-color: #2563eb; color: #ffffff; padding: 12px 28px; text-decoration: none; border-radius: 9999px; font-weight: 600; font-size: 15px; display: inline-block; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
            ${content.buttonText}
          </a>
        </div>
        <div style="margin-top: 24px; padding-top: 16px; border-top: 1px solid #f1f5f9;">
